...
class Evaluator(object):
    ...
    def run(self, model_path, model_indice, log_file, log_file_link, modell):
        ...
        if modell is not None: 
            ...
            if len(self.devices) == 1:
                result_line, mIoU = self.single_process_evalutation()
            else:
    ...
    def val_func_process(self, input_data, device=None):
        ...
        with torch.cuda.device(input_data.get_device()):
            ...
            with torch.no_grad():
                X, score = self.val_func(input_data)
                score = score[0]

                if self.is_flip:
                    input_data = input_data.flip(-1)
                    X, score_flip = self.val_func(input_data)
