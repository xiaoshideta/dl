...
parser.add_argument('--distillation_alpha', type=float, default=0.01, help='Description of new argument')
parser.add_argument('--distillation_beta', type=float, default=0.01, help='Description of new argument')
...
parser.add_argument('--distillation_flag', type=int, default=1, help='Description of new argument')
parser.add_argument('--distillation_middle', type=int, default=1, help='Description of new argument')
logger = get_logger()
...
def distill_feature_maps(rgbd_features, rgb_features):
    ...
    # 遍历两个列表中的张量，计算它们之间的损失
    # for rgbd_feature, rgb_feature in zip(rgbd_features, rgb_features):
    # print("rgbd_features: ", rgbd_features.shape)
    # print("rgb_features: ", rgb_features.shape)
        # 计算当前对张量之间的损失并累加到总损失中
...
with Engine(custom_parser=parser) as engine:
    ...
    if args.distillation_middle == 1:
        print("use distillation_middle")
    ...
    for epoch in range(engine.state.epoch, config.nepochs + 1):
        ...
        for idx in pbar:
            ...
            aux_rate = 0.2
            # 输入模型，获得损失
            rgbd_x, logits, loss = model(imgs, modal_xs, gts)
            # print(logist.shape) # [2, 40, 480, 640]
            rgb_x, logits2, loss2 = model2(imgs, None, gts)
            ...
            # distillation_alpha = 0.01
                ...
                loss_drkl = kl_calculator.compute_kl_divergence(logits.detach(), logits2) * args.distillation_alpha
            else:
            ...
            feature_loss = 0.0
            for i in range(4):
                feature_loss += distill_feature_maps(rgbd_x[i], rgb_x[i]) * args.distillation_beta
            # print("feature_loss", feature_loss)
            if args.distillation_middle:
                loss = loss + feature_loss
                # loss2 = loss2 + feature_loss
            else:
                loss = loss
                loss2 = loss2
